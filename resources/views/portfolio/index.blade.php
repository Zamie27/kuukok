                ...
                <div class="form-control w-full max-w-4xl">
                    <form action="{{ route('portfolio.index') }}" method="GET">
                        <div class="join w-full shadow-sm">
                            <select name="sort" class="select select-bordered join-item bg-base-100" onchange="this.form.submit()">
                                <option value="newest" {{ request('sort') == 'newest' ? 'selected' : '' }}>Terbaru</option>
                                <option value="oldest" {{ request('sort') == 'oldest' ? 'selected' : '' }}>Terlama</option>
                            </select>
                            <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari project berdasarkan judul, deskripsi, atau tags (e.g. Web, Mobile)..." class="input input-bordered w-full join-item bg-base-100" />
                            <button type="submit" class="btn btn-square btn-primary join-item text-white">
                    ...
                    <div class="card-body p-6">
                        <div class="mb-2 flex items-center justify-between">
                            ...
                            <span class="text-xs text-base-content/60">
                                {{ $item->start_date ? $item->start_date->translatedFormat('F Y') : ($item->published_at ? $item->published_at->year : $item->created_at->year) }}
                            </span>
                        </div>
                        ...
                        <p class="mb-4 text-sm text-base-content/70">
                            {{ Str::limit($item->excerpt ?? strip_tags($item->description), 100) }}
                        </p>
                        @if(!empty($item->tags))
