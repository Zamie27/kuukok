<x-layouts.admin title="Admin - Edit Portfolio">
    <div class="mx-auto max-w-5xl" x-data="portfolioForm({{ json_encode($portfolio->teamMembers->map(fn($m) => ['profile_id' => $m->id, 'role' => $m->pivot->role])) }})">
                ...
                <form method="POST" action="{{ route('admin.portfolios.update', $portfolio) }}" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="form-control">
                                ...
                                <div class="flex gap-2 items-center">
                                    <input type="text" name="client_name" id="client_name" class="input input-bordered w-full" value="{{ old('client_name', $portfolio->client_name) }}" placeholder="Company or Person Name" />
                                    <label class="cursor-pointer label border rounded px-2 hover:bg-base-200">
                                        <input type="checkbox" name="is_personal_project" value="1" id="personal_project" class="checkbox checkbox-sm checkbox-secondary" onchange="togglePersonalProject(this)"
                                            {{ old('is_personal_project', $portfolio->is_personal_project) ? 'checked' : '' }} />
                            ...
                            <div class="grid grid-cols-1 gap-4">
                                @php
                                $startMonth = old('start_month', $portfolio->start_date?->month);
                                $startYear = old('start_year', $portfolio->start_date?->year);
                                $endMonth = old('end_month', $portfolio->end_date?->month);
                                $endYear = old('end_year', $portfolio->end_date?->year);
                                @endphp
                                <div class="form-control">
                                    ...
                                    <div class="flex gap-2">
                                        <select name="start_month" class="select select-bordered w-1/2">
                                            ...
                                            <option value="{{ $m }}" {{ $startMonth == $m ? 'selected' : '' }}>{{ DateTime::createFromFormat('!m', $m)->format('F') }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-control">
                                    ...
                                    <div class="flex gap-2">
                                        <select name="end_month" class="select select-bordered w-1/2">
                                            <option value="">Month</option>
                                            @foreach(range(1, 12) as $m)
                                            <option value="{{ $m }}" {{ $endMonth == $m ? 'selected' : '' }}>{{ DateTime::createFromFormat('!m', $m)->format('F') }}</option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div class="form-control">
                                    ...
                                    <select name="project_status" class="select select-bordered w-full">
                                        <option value="Dalam Pengerjaan" {{ old('project_status', $portfolio->project_status) == 'Dalam Pengerjaan' ? 'selected' : '' }}>Dalam Pengerjaan</option>
                                        <option value="Selesai" {{ old('project_status', $portfolio->project_status) == 'Selesai' ? 'selected' : '' }}>Selesai</option>
                                        <option value="Ditunda" {{ old('project_status', $portfolio->project_status) == 'Ditunda' ? 'selected' : '' }}>Ditunda</option>
                                    </select>
                                </div>
                                <div class="form-control">
                            ...
                            <div class="form-control" x-data="{
                                ...
                                selectedRoles: {{ json_encode(old('project_roles', $portfolio->project_roles ?? [])) }},
                                roleInput: '',
                                addRole(role) {
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="form-control">
                                ...
                                <input type="url" name="live_demo_link" class="input input-bordered w-full" value="{{ old('live_demo_link', $portfolio->live_demo_link) }}" placeholder="https://" />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="form-control" x-data="{
                                ...
                                selected: {{ json_encode(old('tags', $portfolio->tags ?? [])) }},
                                query: '',
                                filteredTags() {
                        ...
                        <div class="form-control">
                            ...
                            <div class="flex flex-wrap gap-2 p-3 border rounded-lg bg-base-100 h-32 overflow-y-auto">
                                @php $selectedStacks = $portfolio->techStacks->pluck('id')->toArray(); @endphp
                                ...
                                <label class="cursor-pointer label justify-start gap-2">
                                    <input type="checkbox" name="tech_stacks[]" value="{{ $stack->id }}" class="checkbox checkbox-sm checkbox-secondary"
                                        {{ in_array($stack->id, old('tech_stacks', $selectedStacks)) ? 'checked' : '' }} />
                    ...
                    <div class="form-control">
                        ...
                        <textarea name="excerpt" rows="2" class="textarea textarea-bordered w-full" placeholder="Brief summary for cards...">{{ old('excerpt', $portfolio->excerpt) }}</textarea>
                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const input = document.getElementById('client_name');
            if (cb && cb.checked && input.value === 'Personal Project') {
                input.readOnly = true;
                input.dataset.oldValue = '';
            }
        });

        function portfolioForm(existingMembers = []) {
            return {
                members: existingMembers.length ? existingMembers : [],
                addMember() {
                    this.members.push({
                        profile_id: '',
                        role: ''
                ...
                init() {
                    const key = 'portfolio_edit_{{ $portfolio->id }}_autosave';

                    // Check for saved data but don't auto-restore to avoid overwriting DB data
                    // unless specifically requested or if we want to be aggressive.
                    // For now, we will just implement the SAVE part so if they crash, they can manually check localStorage
                    // or we can implement a "Restore" button later.
                    // But to be helpful, let's restore if the field is empty (which shouldn't happen in edit)
                    // OR if we strictly want to support crash recovery.
